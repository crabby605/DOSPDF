...
#define MUL_MACRO(op_data_type,out_regs) (set_opcode(0x10), \
										  out_regs[i_w + 1] = (op_result = CAST(op_data_type)mem[rm_addr] * (CAST_T(op_data_type))*out_regs) >> 16, \
										  regs16[REG_AX] = op_result, \
										  set_OF(set_CF(op_result - (CAST_T(op_data_type))op_result)))
#define DIV_MACRO(out_data_type,in_data_type,out_regs) (scratch_int = CAST(out_data_type)mem[rm_addr]) && !(scratch2_uint = (CAST_T(in_data_type))(scratch_uint = (out_regs[i_w+1] << 16) + regs16[REG_AX]) / scratch_int, scratch2_uint - (CAST_T(out_data_type))scratch2_uint) ? out_regs[i_w+1] = scratch_uint - scratch_int * (*out_regs = scratch2_uint) : pc_interrupt(0)
...
#define R_M_OP(dest,op,src) (i_w ? op_dest = CAST(US)dest, op_result = CAST(US)dest op (op_source = CAST(US)src) \
								 : (op_dest = dest, op_result = dest op (op_source = CAST(UC)src)))
...
#define SIGN_OF(a) (1 & (i_w ? CAST(S)a : a) >> (TOP_BIT - 1))

// Reinterpretation cast
// 8086tiny relies on the host tolerating unaligned multi-byte memory access
// (e.g. reading a 16-bit operand from an odd address). x86 allows this, but
// asm.js compiles *(short*)p to HEAP16[p>>1], which silently drops the low
// address bit and reads the wrong location. This fastcomp clang ignores an
// inline aligned(1) attribute in a cast, but DOES honour it on a typedef, so
// we route every CAST through an aligned(1) typedef (single-token tags so the
// macro can paste them). These compile to byte-wise, alignment-safe accesses.
typedef short __attribute__((aligned(1))) cast_S;
typedef unsigned short __attribute__((aligned(1))) cast_US;
typedef unsigned __attribute__((aligned(1))) cast_U;
typedef int __attribute__((aligned(1))) cast_I;
typedef unsigned char cast_UC;
typedef char cast_C;
// CAST(tag) reinterprets an lvalue through the matching aligned(1) typedef.
// CAST_T(tag) yields just the type (for plain value casts inside MUL/DIV).
#define CAST(a) *(cast_##a *)&
#define CAST_T(a) cast_##a
...
{
	for (int i = 0; i < len; i++)
		stream[i] = (spkr_en == 3) && CAST(US)mem[0x4AA] ? -((54 * wave_counter++ / CAST(US)mem[0x4AA]) & 1) : sdl_audio.silence;

	spkr_en = io_ports[0x61] & 3;
}
#endif

// Expose the base of the emulated RAM so the JS layer can read video memory
// (text buffer at physical 0xB8000) and CPU/BIOS state between ticks.
unsigned char *get_mem_ptr(void)
{
	return mem;
}

// Debug accessors so the JS layer can trace where execution is.
unsigned short get_ip(void)
{
	return reg_ip;
}
unsigned int get_inst_counter(void)
{
	return inst_counter;
}

// Boot device flag: 0 = floppy, 0x80 = hard disk. Default to floppy.
unsigned char boot_from_hd = 0;

// One-time machine setup. Mirrors TinyEMU's vm_start: configures the machine
// and returns immediately (the run loop is driven separately from JS).
int vm_start(void)
{
	...
	regs8[REG_DL] = boot_from_hd ? 0x80 : 0;

	// Open BIOS (file id disk[2]), floppy disk image (disk[1]) from the embedded MEMFS.
	// Hard disk (disk[0]) is unused for now.
	disk[2] = open("bios", 32898);
	disk[1] = open("fd.img", 32898);
	disk[0] = 0;

	// Set CX:AX equal to the hard disk image size, if present
	CAST(U)regs16[REG_AX] = *disk ? lseek(*disk, 0, 2) >> 9 : 0;
...
// Inject a keypress from JS. `code` is the 16-bit value the BIOS INT 7 handler
// reads from 0040:this_keystroke (mem[0x4A6]):
//   bit 0x400 = "from SDL" (rich path, enables modifier bits below)
//   bit 0x800 = Alt, 0x1000 = Shift, 0x2000 = Ctrl, 0x4000 = key-up
//   low bits  = SDL keysym (or ASCII for printable keys)
// A plain ASCII byte (0x400 clear) takes the simpler headless path.
// Raising INT 7 is only safe at an instruction boundary with interrupts
// enabled; returns 0 if the guest currently has interrupts masked, so the JS
// driver can re-queue and retry on a later tick.
int kbd_inject(int code)
{
	if (!regs8[FLAG_IF])
		return 0;
	CAST(S)mem[0x4A6] = (short)code;
	pc_interrupt(7);
	return 1;
}

// Run up to max_steps instructions, returning the number actually executed.
// Returns 0 once the machine halts (CS:IP == 0:0). Matches the cooperative
// interface that the JS driver's machine_tick() expects (_virt_machine_run).
int virt_machine_run(int max_steps)
{
	int steps = 0;

	// Instruction execution loop. Terminates if CS:IP = 0:0
	for (; steps < max_steps && (opcode_stream = mem + 16 * regs16[REG_CS] + reg_ip, opcode_stream != mem); steps++)
	{
		...
		i_data0 = CAST(S)opcode_stream[1];
		i_data1 = CAST(S)opcode_stream[2];
		i_data2 = CAST(S)opcode_stream[3];
		...
		if (i_mod_size)
		{
			...
			if ((!i_mod && i_rm == 6) || (i_mod == 2))
				i_data2 = CAST(S)opcode_stream[4];
			else if (i_mod != 1)
				i_data2 = i_data1;
		...
		switch (xlat_opcode_id)
		{
			...
			OPCODE_CHAIN 5: // INC|DEC|JMP|CALL|PUSH
				if (i_reg < 2) // INC|DEC
					...
					i_reg & 1 && (regs16[REG_CS] = CAST(S)mem[op_from_addr + 2]), // JMP|CALL (far)
			...
			OPCODE 6: // TEST r/m, imm16 / NOT|NEG|MUL|IMUL|DIV|IDIV reg
				...
				switch (i_reg)
				{
					...
					OPCODE 4: // MUL
						i_w ? MUL_MACRO(US, regs16) : MUL_MACRO(UC, regs8)
					OPCODE 5: // IMUL
						i_w ? MUL_MACRO(S, regs16) : MUL_MACRO(C, regs8)
					OPCODE 6: // DIV
						i_w ? DIV_MACRO(US, U, regs16) : DIV_MACRO(UC, US, regs8)
					OPCODE 7: // IDIV
						i_w ? DIV_MACRO(S, I, regs16) : DIV_MACRO(C, S, regs8);
				}
			...
			OPCODE 21: // IN AL/AX, DX/imm8
				...
				scratch_uint == 0x3D5 && (io_ports[0x3D4] >> 1 == 7) && (io_ports[0x3D5] = ((mem[0x49E]*80 + mem[0x49D] + CAST(S)mem[0x4AD]) & (io_ports[0x3D4] & 1 ? 0xFF : 0xFF00)) >> (io_ports[0x3D4] & 1 ? 0 : 8)); // CRT cursor position
			...
			OPCODE 22: // OUT DX/imm8, AL/AX
				...
				scratch_uint == 0x3D5 && (io_ports[0x3D4] >> 1 == 7) && (scratch2_uint = ((mem[0x49E]*80 + mem[0x49D] + CAST(S)mem[0x4AD]) & (io_ports[0x3D4] & 1 ? 0xFF00 : 0xFF)) + (regs8[REG_AL] << (io_ports[0x3D4] & 1 ? 0 : 8)) - CAST(S)mem[0x4AD], mem[0x49D] = scratch2_uint % 80, mem[0x49E] = scratch2_uint / 80); // CRT cursor position
			...
			OPCODE 48: // Emulator-specific 0F xx opcodes
				switch ((char)i_data0)
				{
					...
					OPCODE 1: // GET_RTC
						...
						CAST(S)mem[SEGREG(REG_ES, REG_BX, 36+)] = ms_clock.millitm;
					OPCODE 2: // DISK_READ
					OPCODE_CHAIN 3: // DISK_WRITE
						regs8[REG_AL] = ~lseek(disk[regs8[REG_DL]], CAST(U)regs16[REG_BP] << 9, 0)
		...
		// then process the tick. Keyboard input is injected separately from JS (Step C),
		// so the blocking read(0,...) keyboard driver is intentionally omitted here.
		if (int8_asap && !seg_override_en && !rep_override_en && regs8[FLAG_IF] && !regs8[FLAG_TF])
			pc_interrupt(0xA), int8_asap = 0;
	}

	return steps;
}

// Emscripten requires a main symbol; the real driving happens via the
// exported vm_start / virt_machine_run called from JS.
